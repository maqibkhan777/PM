...
async def test_slash_command_report_overdue(slash_setup):
    ...
    assert isinstance(res1, dict) and "embeds" in res1
    embed1 = res1["embeds"][0]
    assert "📋" in embed1["title"]
    assert "Overdue Tasks" in embed1["title"]
    assert "[WSSS-326](" in embed1["description"]
    assert "Abdul Subhan" in embed1["description"]
    assert "2026-09-10" in embed1["description"]
    assert embed1["footer"]["text"] == "Generated by PM Operations Agent"
    assert embed1["color"] == 15158332
    ...
    assert isinstance(res2, dict) and "embeds" in res2
    embed2 = res2["embeds"][0]
    assert "📋" in embed2["title"]
    assert "[WSSS-326](" in embed2["description"]
...
async def test_slash_command_report_overdue_empty(slash_setup):
    ...
    assert "No overdue tasks found." in embed["description"]
...
async def test_slash_command_report_attention(slash_setup):
    """Test /pm report name:attention and /pm attention generate PM Attention Digest mobile embed."""
    handler, bot, jira_client, engine, temp_db = slash_setup
    from app.database.repositories import JiraIssueStateRepository
    state_repo = JiraIssueStateRepository(temp_db)
    state_repo.upsert(
    ...
    assert isinstance(res, dict) and "embeds" in res
    embed = res["embeds"][0]
    assert "PM Attention Digest" in embed["title"]
    assert "WSSS-500" in embed["description"]
    assert embed["footer"]["text"] == "Generated by PM Operations Agent"
    ...
    assert isinstance(res_alias, dict) and "embeds" in res_alias
    embed_alias = res_alias["embeds"][0]
    assert "PM Attention Digest" in embed_alias["title"]
    assert "WSSS-500" in embed_alias["description"]
    assert res == res_alias


@pytest.mark.asyncio
async def test_slash_command_report_activity(slash_setup):
    """Test /pm report name:activity and /pm activity generate Daily PM Activity Report embed."""
    handler, bot, jira_client, engine, temp_db = slash_setup
    ...
    assert "**Total Activities:** 1" in embed["description"]
    assert "Aqib Khan" in embed["description"]
    assert embed["footer"]["text"] == "Generated by PM Operations Agent"
...
async def test_slash_command_user_worklog_success_and_empty(slash_setup):
    ...
    wl_repo.upsert_worklog(
        ...
        jira_issue_key="WSSS-326",
        jira_issue_id="1001",
        author_account_id="712020:c12d2371-c035-423a-bc44-30799981aa7a",
    ...
    # 1. Direct alias /pm worklog user:Abdul Subhan
    res = await handler.execute_subcommand(
        subcommand="worklog",
...
async def test_slash_command_user_overdue_success_and_empty(slash_setup):
    ...
    embed = res["embeds"][0]
    assert "📋" in embed["title"]
    assert "Overdue Tasks — Abdul Subhan" in embed["title"]
    assert "[WSSS-401](" in embed["description"]
    assert "2026-09-10" in embed["description"]
    assert "Total Overdue:** 1" in embed["description"]
    assert embed["footer"]["text"] == "Generated by PM Operations Agent"
    ...
    assert isinstance(res_empty, dict) and "embeds" in res_empty
    embed_empty = res_empty["embeds"][0]
    assert "📋" in embed_empty["title"]
    assert "No overdue tasks found for Ahsan Amin." in embed_empty["description"]
    assert embed_empty["color"] == 3066993
...
async def test_slash_command_user_queue_canonical(slash_setup):
    ...
    embed = res["embeds"][0]
    assert "📋" in embed["title"]
    assert "Active Queue — Abdul Subhan" in embed["title"]
    assert "[WSSS-501](" in embed["description"]
    assert "Fix payment gateway timeout" in embed["description"]
    assert "WSSS-502" not in embed["description"], "Done status must NOT appear in Active Queue"
    assert "Active Tasks:** 1" in embed["description"]
    assert embed["footer"]["text"] == "Generated by PM Operations Agent"
...
class TestPhase1SlashCommandCleanup:
    ...
    async def test_report_overdue_routes_to_canonical_overdue(self, slash_setup):
        ...
        assert res_direct == res_shim
        assert isinstance(res_shim, dict) and "embeds" in res_shim
        assert "[WSSS-326](" in res_shim["embeds"][0]["description"]
...
class TestDailyActivityReportEnhancements:
    """Focused tests for Daily PM Activity Report Mursaleen Cluster filtering and Jira links."""

    @pytest.mark.asyncio
    async def test_activity_report_contains_clickable_jira_links_and_filters_members(self, slash_setup):
        """Verify activity report contains only cluster member actions with clickable Jira markdown links."""
        handler, _, _, _, temp_db = slash_setup
        from app.database.repositories import EventRepository, EmployeeRoleRepository
        role_repo = EmployeeRoleRepository(temp_db)
        role_repo.upsert_assignment(
        ...
        event_repo = EventRepository(temp_db)
        # 1. Included activity: Ahsan Amin transition
        event_repo.insert(
            event_id="act-ev-1",
            event_type="TaskStatusChanged",
            source="jira",
            external_event_id="act-ext-1",
            timestamp="2026-09-12T10:00:00Z",
            actor_name="Ahsan Amin",
            task_id="WSSS-326",
            payload={"old_status": "To Do", "new_status": "In Progress"}
        )
        # 2. Included activity: Mubashir Butt comment
        event_repo.insert(
            event_id="act-ev-2",
            event_type="TaskCommentAdded",
            source="jira",
            external_event_id="act-ext-2",
            timestamp="2026-09-12T11:00:00Z",
            actor_id="712020:e268bcd8-d981-4b4d-992d-d5694745df8b",
            actor_name="Mubashir Butt",
            task_id="SMTPSUPORT-123",
            payload={"comment": "Client update"}
        )
        # 3. Excluded activity: QA Runner (service account)
        event_repo.insert(
            event_id="act-ev-3",
            event_type="TaskCreated",
            source="jira",
            external_event_id="act-ext-3",
            timestamp="2026-09-12T12:00:00Z",
        ...
        # Only 2 qualifying activities (QA Runner excluded)
        assert "**Total Activities:** 2" in desc
        assert "**Active Members:** 2" in desc
        assert "Ahsan Amin" in desc
        assert "Mubashir Butt" in desc
        assert "QA Runner" not in desc
