...
def test_format_daily_worklog_embed_focused_2026_09_14():
    """Focused formatter test using the actual 2026-09-14 report shape."""
    import re
    actual_2026_09_14_report = {
        "team_name": "Mursaleen Cluster",
        "report_date": "2026-09-14",
        "formatted_date": "September 14, 2026",
        "total_time_seconds": 214620,
        "total_time_human": "59h 37m",
        "members_logged_count": 12,
        "reportable_members_count": 18,
        "tickets_worked_count": 68,
        "tickets": [
            # 68 tickets whose worklogs_count sum to 86
            {"issue_key": f"TICKET-{i}", "worklogs_count": 2 if i < 18 else 1}
            for i in range(68)
        ],
        "members": [
            {"display_name": "Hamza Hanif", "time_logged_seconds": 28500, "time_logged_human": "7h 55m", "tickets_count": 16, "tickets": [f"WPEP-{i}" for i in range(16)]},
            {"display_name": "Mubashir Butt", "time_logged_seconds": 25560, "time_logged_human": "7h 06m", "tickets_count": 26, "tickets": [f"PP-{i}" for i in range(26)]},
            {"display_name": "Daniyal Raza", "time_logged_seconds": 24900, "time_logged_human": "6h 55m", "tickets_count": 5, "tickets": [f"TREN-{i}" for i in range(5)]},
            {"display_name": "shoaib hassan askari", "time_logged_seconds": 23400, "time_logged_human": "6h 30m", "tickets_count": 1, "tickets": ["AIOL-101"]},
            {"display_name": "Muneeb Jalal", "time_logged_seconds": 21900, "time_logged_human": "6h 05m", "tickets_count": 3, "tickets": [f"HFCF-{i}" for i in range(3)]},
            {"display_name": "Ahsan Amin", "time_logged_seconds": 21600, "time_logged_human": "6h", "tickets_count": 2, "tickets": ["WSSS-101", "WSSS-102"]},
            {"display_name": "Muhammad Ali Siddiqui", "time_logged_seconds": 18300, "time_logged_human": "5h 05m", "tickets_count": 6, "tickets": [f"CF7-{i}" for i in range(6)]},
            {"display_name": "Azain Hassan", "time_logged_seconds": 18300, "time_logged_human": "5h 05m", "tickets_count": 8, "tickets": [f"POST-{i}" for i in range(8)]},
            {"display_name": "Muhammad Usama Azad", "time_logged_seconds": 15300, "time_logged_human": "4h 15m", "tickets_count": 3, "tickets": [f"INV-{i}" for i in range(3)]},
            {"display_name": "Tahir Ali", "time_logged_seconds": 14400, "time_logged_human": "4h", "tickets_count": 4, "tickets": [f"PAY-{i}" for i in range(4)]},
            {"display_name": "Muhammad Shahmeer Khan", "time_logged_seconds": 13800, "time_logged_human": "3h 50m", "tickets_count": 3, "tickets": [f"AUTH-{i}" for i in range(3)]},
            {"display_name": "Nauman Sadiq", "time_logged_seconds": 7200, "time_logged_human": "2h", "tickets_count": 1, "tickets": ["OPS-101"]},
        ],
        "zero_worklog_members": [
    ...
    payload = DiscordFormatter.format_daily_worklog_embed(actual_2026_09_14_report)
    ...
    # 3. Description Character count below Discord limit (4096)
    desc = embed["description"]
    assert len(desc) < 4096
    assert 2000 < len(desc) < 3000

    # 4. Header lines
    assert "**Date:** 2026-09-14" in desc
    assert "**Tickets Worked:** 68" in desc
    assert "**Total Worklogs:** 86" in desc
    assert "**Active Members:** 12 / 18" in desc
    assert "👥 **Team Worklog**" in desc

    # 5. All 12 active members retained
    expected_active = [
        "Hamza Hanif", "Mubashir Butt", "Daniyal Raza", "shoaib hassan askari",
        "Muneeb Jalal", "Ahsan Amin", "Muhammad Ali Siddiqui", "Azain Hassan",
        "Muhammad Usama Azad", "Tahir Ali", "Muhammad Shahmeer Khan", "Nauman Sadiq"
    ]
    for m in expected_active:
        assert m in desc

    # 6. Clickable Jira Navigator links with singular/plural support
    assert "Mubashir Butt — 7h 06m — [26 tickets](https://" in desc
    assert "shoaib hassan askari — 6h 30m — [1 ticket](https://" in desc
    assert "Nauman Sadiq — 2h — [1 ticket](https://" in desc

    # 7. Zero raw URLs visible
    url_matches = list(re.finditer(r"https?://\S+", desc))
    assert len(url_matches) == 12
    for match in url_matches:
        start_pos = match.start()
        end_pos = match.end()
        assert desc[max(0, start_pos - 2):start_pos] == "]("
        assert desc[end_pos - 1] == ")" or desc[end_pos:end_pos + 1] == ")"

    # 8. The 6 zero-work members visible
    assert "⏸️ **No Logged Work**" in desc
    expected_zero = [
        "Ahsan Iftikhar", "Muhammad Bilal Khan", "Muhammad Hamza",
        "Muhammad Sufiyan", "Syed ali", "Talha Bukhari"
    ]
    for zm in expected_zero:
        assert zm in desc

    # 9. When no zero-work members exist, section is omitted cleanly
    no_zero_report = dict(actual_2026_09_14_report)
